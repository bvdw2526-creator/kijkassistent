...
export type RecommendationMode = 'focused' | 'balanced' | 'explore'
...
export interface RankedCandidate extends TmdbItem {
  score: number
  matchPercent: number
  basedOn: string[]
  coreScore: number
...
// Zet ruwe kijkproviders om in het display-vriendelijke "watchOn/watchUrl"-veld voor
// alle meegegeven items, gefilterd op de streamingdiensten van de gebruiker(s). Gedeeld
// door de gewone en de "samen"-aanbevelingsroute, zodat de sub/rent/buy-voorkeurslogica
// niet dubbel hoeft te bestaan.
...
async function fetchCreditsLive(mediaType: MediaType, tmdbId: number): Promise<CreditsResult | null> {
  ...
  try {
    const res = await fetch(
      `https://api.themoviedb.org/3/${endpoint}/${tmdbId}/credits`,
      { headers: { Authorization: `Bearer ${process.env.TMDB_API_KEY}` } }
    )
    if (!res.ok) return null
    const data = await res.json()
...
// Zoekt titels via TMDB's discover-endpoint die bij de meegegeven genres passen.
// Losgetrokken van de GET-handler (i.p.v. een closure over "supabase") zodat zowel de
// gewone als de "samen"-aanbevelingsroute 'm kunnen hergebruiken.
...
export function buildProfileSignature(inputs: ProfileInputs): string {
  return [
    ...
    'rat:' + inputs.ratings.map((r) => `${r.media_type}-${r.tmdb_id}-${r.rating}`).sort().join(','),
    'wl:' + inputs.watchlist.map((w) => `${w.media_type}-${w.tmdb_id}`).sort().join(','),
...
const EMPTY_SORTED_BY_MODE: Record<RecommendationMode, RankedCandidate[]> = {
  focused: [],
  balanced: [],
  explore: [],
}
...
// drie tabbladen met gerangschikte, van een matchpercentage voorziene kandidaten. Dit is
// letterlijk wat de gewone aanbevelingsroute nodig heeft, en wat de "samen"-route twee
...
export async function computeTasteProfile(
  ...
  function allGenres(mediaType: MediaType): GenreAffinity[] {
    return Array.from(genreCounts[mediaType].entries())
      .map(([id, v]) => ({ id, ...v }))
      // Uitgesloten genres hier al wegfilteren: anders blijft de discover-stap er
      // juist meer van dit genre bij zoeken, en krijgt de round-robin er slots voor.
      .filter((g) => !excludedGenreIds.has(g.id))
  ...
  function addScore(items: TmdbItem[], field: ScoreField, weight: number, label: string) {
    for (const item of items) {
      ...
      // Uitgesloten genres nooit als kandidaat laten instromen, ook niet via een
      // collectie- of titel-aanbeveling — zie excludedGenreIds hierboven.
